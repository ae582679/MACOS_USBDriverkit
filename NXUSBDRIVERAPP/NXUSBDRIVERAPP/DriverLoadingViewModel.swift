...
class DriverLoadingViewModel: NSObject {
    ...
    public var dextLoadingState: String {
        switch state {
        case .unloaded:
            return "NXUSB Driver isn't loaded."
        case .activating:
            return "Activating NXUSB Driver, please wait."
        case .needsApproval:
            return "Please follow the prompt to approve NXUSB Driver."
        case .activated:
            return "NXUSB Driver has been activated and is ready to use."
        case .activationError:
            return "NXUSB Driver has experienced an error during activation.\nPlease check the logs to find the error."
        }
    }
}
